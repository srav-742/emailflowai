...
async function extractTasksWithAI(email = {}) {
  ...
  const prompt = [
    ...
    '2. Each object must have: "id" (unique string), "task" (clear description), "deadline" (ISO DATE YYYY-MM-DD or null), "priority" (low|medium|high), "assignee" (me|sender|other), "completed" (false).',
    '3. If the sender asks for something (e.g. "Can you send the report?"), the assignee is "me".',
    '4. If YOU promise something (e.g. "I will send it by Friday"), the assignee is "me".',
    '5. If the deadline is implicit (e.g. "by EOD tomorrow"), resolve it to an actual YYYY-MM-DD date based on today being ' + new Date().toDateString() + '.',
    '6. If NO actionable items are found, return exactly [].',
    '',
